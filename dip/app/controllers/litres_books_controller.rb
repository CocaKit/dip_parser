class LitresBooksController < ApplicationController
	...
	def index
		@page = session[:litres_current_page]
		if not @page.nil?
	...
	def page
		page = params[:id].to_i
		session[:litres_current_page] = page
		@litres_books = @litres_books.limit(10).offset(10 * (page - 1))
		respond_to do |format|
	...
	def set_litres_books_from_session
		@atr = session[:litres_searched_books]
		if not @atr.nil?
			...
			@litres_books = @litres_books.order(@atr["order"])
		end
	end
	...
	def set_litres_books_from_params
		...
		@litres_books = @litres_books.order(params[:order])
		session[:litres_searched_books] = params
	end
